...
3.  绑定自定义事件： 
   ...
   -  第一种方式，在父组件中： `<Demo @xiubai='text'/>`或`<Demo v-on:xiubai='text'/>`
   ...
   - 第二种方式，在父组件中：
     ...
     ......
     mounted(){
        this.$refs.xxx.$on('xiubai',this.test)
     }
     ```

4. 触发自定义事件：`this.$emit('xiubai',数据)`

5. 解绑自定义事件`this.$off('xiubai')`

6. 组件上也可以绑定原生DOM事件，需要使用`native`修饰符。

7. 注意：通过`this.$refs.xxx.$on('xiubai',回调)`绑定自定义事件时，回调（<span style="color:red">**要么配置在`methods`中**</span>，<span style="color:red">**要么用箭头函数**</span>，否则this指向会出问题！
...
3.  使用事件总线： 
   ...
   - 接收数据：A组件想接收数据，则在A组件中给$bus绑定自定义事件，事件的<span style="color:red;">回调留在A组件自身</span>。
     ...
     ......
     mounted() {
       this.$bus.$on('xxxx',this.demo)//订阅消息
     }
     ```
...
2. 图示：
   ...
   [![pPpDJ3R.png](https://s1.ax1x.com/2023/07/30/pPpDJ3R.png)](https://imgse.com/i/pPpDJ3R)
...
5.  开启命名空间后，组件中调用dispatch 
   ...
   ```js
   //方式一：自己直接dispatch
   this.$store.dispatch('personAbout/addPersonBai',person)
   //方式二：借助mapActions：
...
3. 全局守卫:
   ...
   router.beforeEach((to,from,next)=>{
   	...
   	if(to.meta.isAuth){ //判断当前路由是否需要进行权限控制
   		if(localStorage.getItem('school') === '修白'){ //权限控制的具体规则
   			next() //放行
   		}else{
   			alert('无权限查看')
   		}
